...
class StemBorerList extends StatelessWidget {
  const StemBorerList({super.key});

  // Function to fetch brown marmorated stink bugs data from Firestore
  Future<Map<String, dynamic>?> _fetchStemBorerData() async {
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Stem Borer Information', style: TextStyle(fontSize: 20, letterSpacing: 1)),
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (BuildContext context,
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            Map<String, dynamic> stemborerData = snapshot.data!;
            String description =
                stemborerData['description'] ?? 'No description available';
            String habitat =
                stemborerData['habitat'] ?? 'No habitat information available';
            String impact =
                stemborerData['impact'] ?? 'No impact information available';
  ...
  Widget _buildImageSection() {
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(20.0), // Comma added here
        image: const DecorationImage(
          image: AssetImage('assets/stemborer.jpg'),
          fit: BoxFit.cover,
        ),
      ),
    );
